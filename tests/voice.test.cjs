...
function fixture({speech=true,denied=false,policy=true,suspended=false}={}){
  ...
  const env={isSecureContext:true,document:{permissionsPolicy:{allowsFeature:()=>policy}},navigator:{mediaDevices:{getUserMedia:()=>denied?Promise.reject(Object.assign(new Error(),{name:'NotAllowedError'})):new Promise((a,b)=>{resolve=a;reject=b})}},
...
test('iframe policy is detected before microphone request',async()=>{const f=fixture({policy:false});await f.voice.start();assert.equal(f.voice.state,'error');assert.match(f.states.at(-1)[1],/browser tab/)});
